...
def init_routes(app):
           ...
           for file in request.files.getlist('files'):
               if file and file.filename:
                   ...
                   if embedding is not None:
                       ...
                       # Store in upload_progress table
                       progress_id = str(uuid.uuid4())
                       supabase.table('upload_progress').insert({
                           'id': progress_id,
                           'client_id': client_id,
                           'filename': filename,
                           'status': 'processing',
                       ...
                       # Update progress
                       supabase.table('upload_progress').update({
                           'status': 'completed'
                   ...
                   try:
                       ...
                       if existing_podcast.data:
                           ...
                           new_podcast = {
                               ...
                               "global_rank": float(row['Global Rank'].strip('%') or 0) / 100,
   ...
   @app.route('/get_upload_progress')
   def get_upload_progress():
       try:
           progress_id = request.args.get('progress_id')
           if not progress_id:
               return jsonify({"error": "Progress ID is required"}), 400

           progress = supabase.table('upload_progress')\
               .select('*')\
               .eq('id', progress_id)\
               .execute()
           ...
           return jsonify(progress.data[0])
       except Exception as e:
